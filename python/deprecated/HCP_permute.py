...
if not op.isfile('RMS_{}.txt'.format(session)):
    np.savetxt('RMS_{}.txt'.format(session), RelRMSMean)

# Generate permutations
repetitions = 2
if not op.isfile('permuted_scores.txt') or config.overwrite:
    pscores = np.vstack([random.permutation(score) for i in range(repetitions)])
    np.savetxt('permuted_scores.txt', pscores)
else:
    pscores = np.loadtxt('permuted_scores.txt',dtype=np.int16)
for iSample in range(repetitions):
    # submit jobs with sge
    config.queue        = True
    smpl_score = pscore[iSample,:]
    # # Do work
    ...
    smpl_motscore    = RelRMSMean[keepSub]

    checkProgress(pause=20)

    print 'Computing FC...'
    fcMatFile = 'fcMats_{}_{}_{}'.format(config.pipelineName, config.parcellationName, 'perm{}'.format(iSample))
    ...
    for model in ['IQ']:
        runPredictionPar(fcMatFile,thresh=0.01, model=model,predict=predict, motFile=motFile, idcode='perm{}'.format(iSample))
        if config.queue: 
        ...
        for subject in smpl_subjects:
           results = sio.loadmat(op.join(config.DATADIR, '{}_{}pred_{}_{}_{}_{}.mat'.format(model,predict,config.pipelineName, config.parcellationName, subject, 'perm{}'.format(iSample)))) 
        ...
        results['p_neg'] = p
        # save result
        sio.savemat(op.join(config.DATADIR, '{}_{}pred_{}_{}_{}.mat'.format(model,predict,config.pipelineName, config.parcellationName, 'perm{}'.format(iSample))),results)
